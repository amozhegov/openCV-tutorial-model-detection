...
int main(int argc, char ** argv) {
    ...
    cv::CommandLineParser parser(argc, argv, "{@model | ../data/opencv3_model.png | input model image}"
                                             "{@scene | ../data/opencv3_scene.png | input scene image}"
                                             "{@output | ../data/opencv3_located.png | output image}");
    ...
    rva_calcKPsDesc(img_model, keypoints_model, descriptors_model);

    // Compute keypoints and descriptors for the scene image
    std::vector<cv::KeyPoint> keypoints_scene;
    cv::Mat descriptors_scene;
    rva_calcKPsDesc(img_scene, keypoints_scene, descriptors_scene);
    ...
    rva_locateObj(img_model, img_scene, keypoints_model, keypoints_scene, matches, H, pts_obj_in_scene);
    ...
    cv::imwrite(parser.get<cv::String>("@output"), img_scene_col);
